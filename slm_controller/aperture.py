...
class DigitalAperture:
    def __init__(self, slm_dim):
        ...
        ----------
        slm_dim : tuple(int)
            Dimensions (height x width) of the spatial light modulator.
        """
        assert slm_dim[0] > 0
        assert slm_dim[1] > 1
    ...
    def mask(self):
        """
        Return mask as an :py:class:`~numpy.ndarray` object.
        """
        return self._mask
...
class RectAperture(DigitalAperture):
    def __init__(self, apert_dim, slm_dim, center=None):
        ...
        ----------
        apert_dim : tuple(int)
            Dimensions (height x width) of aperture.
        slm_dim : tuple(int)
            Dimensions (height x width) of the spatial light modulator (SLM).
        center : tuple(int)
        ...
        top_left = (center[0] - apert_dim[0] // 2, center[1] - apert_dim[1] // 2)
        bottom_right = (top_left[0] + apert_dim[0], top_left[1] + apert_dim[1])
        if (
            top_left[0] < 0
            or top_left[1] < 0
