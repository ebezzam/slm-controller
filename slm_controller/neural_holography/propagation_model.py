...
from pytorch_prototyping.pytorch_prototyping import Conv2dSame
...
class ContentDependentField(nn.Module):
    def __init__(self, num_layers=5, num_features=32, norm=nn.GroupNorm, latent_coords=False):
        ...
        net += [nn.LeakyReLU(0.2, True), Conv2dSame(num_features, 2, kernel_size=3)]
...
class SourceAmplitude(nn.Module):
    def __init__(self, num_gaussians=3, init_sigma=None, init_amp=0.7, x_s0=0.0, y_s0=0.0):
        ...
        if init_sigma is None:
            init_sigma = [100.0] * self.num_gaussians  # default to 100 for all

        # create parameters for source amplitudes
        self.sigmas = nn.Parameter(torch.tensor(init_sigma), requires_grad=True)
        self.x_s = nn.Parameter(torch.ones(num_gaussians) * x_s0, requires_grad=True)
        self.y_s = nn.Parameter(torch.ones(num_gaussians) * y_s0, requires_grad=True)
        self.amplitudes = nn.Parameter(
            torch.ones(num_gaussians) / (num_gaussians) * init_amp, requires_grad=True
        )

        self.dc_term = nn.Parameter(torch.zeros(1), requires_grad=True)
...
class ModelPropagate(nn.Module):
    ...
    def __init__(
        ...
        target_field=True,
        num_gaussians=3,
        init_sigma=(1300.0, 1500.0, 1700.0),
        init_amp=0.9,
        num_coeffs=0,
        num_coeffs_fourier=5,
        init_coeffs=0.0,
        use_conv1d_mlp=True,
        num_layers=3,
        num_features=16,
        num_latent_codes=None,
        norm=nn.GroupNorm,
        blur=None,
        content_field=True,
        num_layers_cdp=5,
        ...
        if num_gaussians:
            self.source_amp = SourceAmplitude(
                num_gaussians, init_sigma, init_amp=init_amp, x_s0=0.0, y_s0=0.0
            )
        else:
            self.source_amp = None
        if target_field:
            self.target_constant_amp = nn.Parameter(
                0.07 * torch.ones(1, 1, *image_res), requires_grad=True
            )
            self.target_constant_phase = nn.Parameter(
                torch.zeros((1, 1, *image_res)), requires_grad=True
            )
        else:
        ...
        if num_coeffs:
            self.coeffs = nn.Parameter(torch.ones(num_coeffs) * init_coeffs, requires_grad=True)
        else:
            self.coeffs = None
        if num_coeffs_fourier:
            self.coeffs_fourier = nn.Parameter(
                torch.ones(num_coeffs_fourier) * init_coeffs, requires_grad=True
            )
        else:
        ...
        if use_conv1d_mlp:
            self.process_phase = LatentCodedMLP(
                num_layers, num_features, norm=norm, num_latent_codes=num_latent_codes
            )
        else:
        ...
        if sum(num_latent_codes) > 0:
            self.latent_code = nn.Parameter(
                torch.zeros(1, sum(num_latent_codes), *image_res), requires_grad=True
            )
        else:
        ...
        if content_field:
            self.content_dependent_field = ContentDependentField(
                num_layers=num_layers_cdp,
                num_features=num_feats_cdp,
                norm=nn.GroupNorm,
        ...
        if latent_coords:
            ...
            self.latent_coords = nn.Parameter(
                torch.from_numpy(np.stack((lx, ly), 0))
                .type(torch.float32)
                .reshape(1, 2, *image_res),
                requires_grad=False,
            )
        else:
        ...
        if learn_dist:
            self.distance = nn.Parameter(
                torch.tensor(distance, dtype=torch.float), requires_grad=True
            )
        else:
        ...
        self.feature_size = feature_size if hasattr(feature_size, "__len__") else [feature_size] * 2
    ...
    def forward(self, phases, skip_lut=False, skip_tm=False):
        ...
        if self.process_phase is not None and not skip_lut:
            if self.latent_code is not None:
                # support mini-batch
                processed_phase = self.process_phase(
                    phases, self.latent_code.repeat(phases.shape[0], 1, 1, 1)
                )
            else:
        ...
        if self.zernike is None and self.coeffs is not None:
            self.zernike = compute_zernike_basis(
                self.coeffs.size()[0], phases.size()[-2:], wo_piston=True
            )
        ...
        if (
            not self.training
            and self.zernike_eval_fourier is None
            and self.coeffs_fourier is not None
        ):
            # sum the phases
            self.zernike_eval_fourier = combine_zernike_basis(
                self.coeffs_fourier, self.zernike_fourier
            )
            self.zernike_eval_fourier = utils.ifftshift(self.zernike_eval_fourier)
            self.zernike_eval_fourier = self.zernike_eval_fourier.to(
                self.coeffs_fourier.device
            ).detach()
        ...
        if self.training:
            if self.coeffs_fourier is None:
                output_complex = self.prop_zernike(
                    processed_complex,
                    self.feature_size,
                    self.wavelength,
                    self.distance,
                ...
                output_complex = self.prop_zernike_fourier(
                    processed_complex,
                    self.feature_size,
                    self.wavelength,
                    self.distance,
        ...
        if self.content_dependent_field is not None:
            if self.latent_coords is not None:
                cdf = self.content_dependent_field(
                    phases, self.latent_coords.repeat(phases.shape[0], 1, 1, 1)
                )
            else:
