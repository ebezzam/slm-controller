...
from slm_controller.hardware import SlmDisplayDevices, SlmParam, slm_display_devices
...
class SummaryModelWriter(tensorboardX.SummaryWriter):
    ...
    def add_source_amplitude_image(self, idx=0):
        ...
        if self.model.source_amp is not None:
            img = (
                self.model.source_amp(torch.empty(1, 1, *self.slm_res).to(self.model.dev))
                .squeeze()
                .cpu()
                .detach()
                .numpy()
        ...
        if zernike_coeffs is not None:
            ...
            if domain == "fourier":
                if self.model.zernike_fourier is None:
                    self.model.zernike_fourier = compute_zernike_basis(
                        self.model.coeffs_fourier.size()[0], map_size, wo_piston=True
                    )
                    self.model.zernike_fourier = self.model.zernike_fourier.to(
                        self.model.dev
                    ).detach()
    ...
    def add_zeroth_order(self, idx=0):
        ...
        recon_amp = (
            utils.crop_image(recon_amp, self.slm_res, stacked_complex=False)
            .cpu()
            .detach()
            .squeeze()
            .unsqueeze(0)
    ...
    def add_latent_codes(self, idx=0, chs=(0, 1)):
        ...
        if self.model.latent_code is not None:
            for ch in chs:
                ...
                self.add_figure_cmap(
                    f"parameters/latent_code/{ch}",
                    lc.cpu().detach().squeeze().numpy(),
                    idx,
                    plt.cm.plasma,
                )
